# IronMen — push to GitHub then deploy on Render (free HTTPS hosting)
param(
  [string]$GitHubUser = "",
  [string]$RepoName = "ironmen"
)
...
if ($LASTEXITCODE -ne 0) {
  git commit -m "IronMen: website + PWA app ready for deploy"
}

if (-not $GitHubUser) {
  Write-Host ""
  Write-Host "=== IronMen deploy ===" -ForegroundColor Cyan
  Write-Host "1. Create a new repo at https://github.com/new (name: ironmen, private or public)"
  ...
  Write-Host "4. After deploy, change the director PIN in Director Settings."
  exit 0
}
